...
async def button_handler(update, context):
    ...
    if query.prays == "infor":
        ...
        room_image_url = "image/Blend makeup.png"
        caption = "салон"
        try:
        ...
        await query.message.reply_text(
            room_image_url="image/makeup.jpg, image/manicure.jpg, image/peducure.jpg"
        caption = "макияж, маникюр, педикюр"
        try:
            await query.message.reply_photo(photo=room_image_url, caption=caption)
        except FileNotFoundError as e:
            await query.message.reply_text(f"Помилка: файл {e.filename} не знайдено.")
...
async def send_photos(update, context):
    photo_paths = ["image/Blend makeup.png", "image/makeup.jpg", "image/manicure.jpg", "image/peducure.jpg"]
    try:
